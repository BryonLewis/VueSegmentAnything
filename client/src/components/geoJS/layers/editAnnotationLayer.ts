...
import { reOrdergeoJSON } from "../geoJSUtils";

export type EditAnnotationTypes = "polygon";
...
export default class EditAnnotationLayer {
  ...
  async changeData(polygon: GeoJSON.Polygon, ) {
    if (this.skipNextExternalUpdate === false) {
      ...
      if (this.geoViewerRef.interactor().mouse().buttons.left) {
        this.leftButtonCheckTimeout = window.setTimeout(() => this.changeData(polygon), 20);
      } else {
        clearTimeout(this.leftButtonCheckTimeout);
        this.formatData(polygon);
      }
    } else {
   ...
   */
  formatData(polygon: GeoJSON.Polygon) {
    this.selectedHandleIndex = -1;
    this.hoverHandleIndex = -1;
    this.event("update:selectedIndex", {
      selectedIndex: this.selectedHandleIndex,
    ...
    if (polygon) {
      ...
      this.setMode("polygon", annotation);
   ...
   */
  highlightStyle() {
    if (this.type === "polygon" || this.type === "Polygon") {
      return {
        handles: {
          rotate: false,
        },
